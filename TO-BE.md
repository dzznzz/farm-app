...
- 06/18 적용. 상단 5개 탭 제거 → 단일 화면으로 통합. 추가 input 위에 "관리 항목" select(바텀시트 `SelectModal` 재사용)로 MAIN_CODE(작물/품종/사이즈/단위/비용) 전체를 노출해 선택. 품종·사이즈는 부모 작물 칩 selector를 함께 표시. MAIN_META로 항목별 메타(라벨·placeholder·부모·범위입력) 일원화.
